...
const toMongoSchema = (options) => {
    ...
    const { schemaInterface } = parseVars.parseMongoVarlist(properties, uniqueIdentifiers);
    ...
    const schema = `
        ...
        export const ${utils.capitalizeFirstLetter(name)}Model = mongoose.model<${name}Doc>('${utils.capitalizeFirstLetter(name)}Model', ${name}Schema)
        // ${comment}
`;
...
const createMongoDBSchema = ({ options }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        logger_1.default.debug("FROM: EPB_server: Failed to add a document export to interface file - possibly because the file doesn't exist.");
    }
    let error = yield writeSchemaToFile(options.name, mongoDBSchema);
    if (error)
        return error;
    logger_1.default.http("FROM: EPB-server: Add a model export statement to db/schemas/index.ts..");
    yield utils.addExportStatement("db/schemas", `export * from "./${schemaName}"`);
    yield utils.alterConfigFile("add", "dbSchemas", schemaName);
